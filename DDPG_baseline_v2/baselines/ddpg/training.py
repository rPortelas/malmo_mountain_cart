...
def run_agent(env, nb_epochs, nb_epoch_cycles, render_eval, reward_scale, render, param_noise, actor, critic,
    ...
    obs_shape = (obs_shape[0]+env.unwrapped.dim_goal,)
    ...
    with U.single_threaded_session() as sess:
        ...
        agent.actor.set_initial_weights(sess)
        sess.graph.finalize()
        ...
        if study == 'GEP_PG' or study == 'GEP_FPG':
            ...
            for i in range(size_buffer):
                agent.store_transition(np.array(buffer_gep[0][i]), np.array(buffer_gep[1][i]),
                                       buffer_gep[2][i][0], np.array(buffer_gep[3][i]), buffer_gep[4][i], update_obs_stats=True)
        ...
        agent.reset()
        env_out = env.reset()
        obs = env_out['observation']
        goal = env_out['desired_goal']

        if eval_env is not None:
            eval_env_out = eval_env.reset()
            eval_obs = eval_env_out['observation']
            eval_goal = eval_env_out['desired_goal']
        done = False
        episode_reward = 0.
        episode_step = 0
        episodes = 0
        t = size_buffer # t starts at size buffer if a buffer is provided, 0 otherwise
        ...
        max_steps = nb_rollout_steps*nb_epoch_cycles*nb_epochs + size_buffer
        ...
        for epoch in range(nb_epochs):
            ...
            for cycle in range(nb_epoch_cycles):
                ...
                eval_episode_rewards_history, eval_goal = evaluate(agent, eval_obs, max_action, eval_env, render_eval, sess, t,
                                                                   eval_episode_rewards_history,
                                                                   best_score, data_path, saver_best, nb_eval_episodes, study,
                                                                   max_steps, size_buffer, eval_goal
                                                                   )

                # Perform rollouts.
                if study != 'GEP_FPG':
                    agent, env, episode_reward, episode_step, epoch_actions, epoch_qs, epoch_episode_rewards, episode_rewards_history, \
                    epoch_episode_steps, epoch_episodes, episodes, goal, t = rollout(nb_rollout_steps, agent, obs, rank,
                                                                                  env, max_action, episode_reward,
                                                                                  episode_step, epoch_actions, epoch_qs,
                                                                                  epoch_episode_rewards, episode_rewards_history,
                                                                                  epoch_episode_steps, epoch_episodes,
                                                                                  episodes, render, t, epoch, goal
                                                                                  )
                # # Train.
...
def rollout(nb_rollout_steps, agent, obs, rank, env, max_action, episode_reward,
            ...
            epoch_episode_steps, epoch_episodes, episodes, render, t, epoch, goal):

    for t_rollout in range(nb_rollout_steps):
        # Predict next action.
        obs_in = np.concatenate([obs, goal])
        action, q = agent.pi(obs_in, apply_noise=True, compute_Q=True)
        ...
        new_obs_dict, r, done, info = env.step(max_action * action)  # scale for execution in env (as far as DDPG is concerned, every action is in [-1, 1])
        new_obs = new_obs_dict['observation']
        new_obs_in = np.concatenate([new_obs, goal])
        t += 1
        if rank == 0 and render:
            env.render()
        episode_reward += r
        ...
        epoch_qs.append(q)
        agent.store_transition(obs_in, action, r, new_obs_in, done)
        obs = new_obs
    ...
    return agent, env, episode_reward, episode_step, epoch_actions,epoch_qs,epoch_episode_rewards, episode_rewards_history, \
            epoch_episode_steps, epoch_episodes, episodes, goal, t
...
def evaluate(agent, eval_obs, max_action, eval_env, render_eval, sess, t, eval_episode_rewards_history,
              best_score, data_path, saver_best, nb_eval_episodes, study, max_steps, size_buffer, eval_goal):

    eval_episode_rewards = []
    eval_qs = []
    if eval_env is not None:
        eval_episode_reward = 0.
        for i in range(nb_eval_episodes):
            while True:
                eval_obs_in = np.concatenate([eval_obs, eval_goal])
                eval_action, eval_q = agent.pi(eval_obs_in, apply_noise=False, compute_Q=True)
                eval_obs_dict, eval_r, eval_done, eval_info = eval_env.step(max_action * eval_action)  # scale for execution in env (as far as DDPG is concerned, every action is in [-1, 1])
               ...
               # print("evaluate eval_done : ", eval_done, " eval_episode_reward:", eval_episode_reward)
                if eval_done:
                    ...
                    eval_episode_rewards_history.append(eval_r)
        ...
        if mpi_mean(eval_episode_rewards) > best_score:
            ...
            for file in os.listdir(data_path+'tf_save/'):
                if path.isfile(data_path+'tf_save/'+file):
    ...
    return agent, eval_episode_rewards, eval_qs, best_score, eval_obs, eval_env, \
            eval_episode_rewards_history, eval_goal