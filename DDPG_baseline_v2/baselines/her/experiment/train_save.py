...
sys.path.append('/projets/flowers/cedric/DDPG_baseline_v2/')
...
def train(policy, rollout_worker, evaluator, env_name, num_cpu,
          ...
          save_policies, rank, gep_memory=None, buffer_location=None, study='HER', **kwargs):
    ...
    if 'GEP' in study:
        ...
        for episode in buffer_gep:
            policy.store_episode(episode)
        size_buff = len(buffer_gep)
        logger.info('Buffer of ' + str(size_buff) + ' has been loaded')
...
def launch(env_name, logdir, n_epochs, num_cpu, seed, replay_strategy, policy_save_interval, clip_return,
    override_params={}, save_policies=True, gep_memory=None, buffer_location=None, study='HER'):
    ...
    train(logdir=logdir, policy=policy, rollout_worker=rollout_worker, evaluator=evaluator, n_epochs=n_epochs,
          ...
          buffer_location=buffer_location, study=study, env_name=env_name, num_cpu=num_cpu)


@click.command()
@click.option('--env_name', type=str, default='FetchPush-v0', help='the name of the OpenAI Gym environment that you want to train on')
