...
def should_skip_env_spec_for_tests(spec):
    ...
    if skip_mujoco and ep.startswith('gym2.envs.mujoco:'):
        return True
    if (    'GoEnv' in ep or
            'HexEnv' in ep or
            ep.startswith('gym2.envs.box2d:') or
            ep.startswith('gym2.envs.box2d:') or
            (ep.startswith("gym2.envs.atari") and not spec.id.startswith("Pong") and not spec.id.startswith("Seaquest"))
    ):
