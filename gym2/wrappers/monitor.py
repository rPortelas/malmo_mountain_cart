...
from gym2 import error, version, logger
import os, json, numpy as np, six
from gym2.wrappers.monitoring import stats_recorder, video_recorder
from gym2.utils import atomic_write, closer
from gym2.utils.json_utils import json_encode_np

FILE_PREFIX = 'openaigym2'
...
class Monitor(Wrapper):
    ...
    def _start(self, directory, video_callable=None, force=False, resume=False,
        ...
        Args:
            ...
            force (bool): Clear out existing training data from this directory (by deleting every file prefixed with "openaigym2.").
        ...
        if self.env.spec is None:
            logger.warn("Trying to monitor an environment which has no 'spec' set. This usually means you did not create it via 'gym2.make', and is recommended only for advanced users.")
            env_id = '(unknown)'
        else:
 ...
 You should use a unique directory for each training run, or use 'force=True' to automatically clear previous monitor files.'''.format(directory, ', '.join(training_manifests[:5])))
        ...
        # We use the 'openai-gym2' prefix to determine if a file is
        # ours
    ...
    def close(self):
        ...
        logger.info('''Finished writing results. You can upload them to the scoreboard via gym2.upload(%r)''', self.directory)
...
# not intended for use outside of the gym2 codebase.
def _open_monitors():
...
def collapse_env_infos(env_infos, training_dir):
    ...
    for key in ['env_id', 'gym2_version']:
        if key not in first:
            raise error.Error("env_info {} from training directory {} is missing expected key {}. This is unexpected and likely indicates a bug in gym2.".format(first, training_dir, key))
    return first