...
def get_state(obs):
    ...
    distractors = fixed_distractor1 + moving_distractor1.tolist() + fixed_distractor2 + moving_distractor2.tolist()
    # ORDER MUST BE LIKE DEFINED INPUT_NAMES
    return np.array([agent_x, agent_y, agent_z, cart_x] + distractors + breads.tolist())
...
def get_outcome(state):
    ...
    #print(len(np.array(s[0:3] + [s[6]] + [sum(s[-len(bread_positions):])] + fixed_distractor1 + moving_distractor1.tolist() + fixed_distractor2+ moving_distractor2.tolist())))
    return  np.array(s[0:-len(bread_positions)] + [sum(s[-len(bread_positions):])])
...
b.add('cart_vx',[-1,1])
# add 1 static and 1 moving distractor if distractor
b.add('fixed_distr_x1', [-1,1])
b.add('fixed_distr_y1', [-1,1])
b.add('fixed_distr_z1', [-1,1])
b.add('moving_distr_x1', [-1,1])
b.add('moving_distr_y1', [-1,1])
b.add('moving_distr_z1', [-1,1])
b.add('fixed_distr_x2', [-1,1])
b.add('fixed_distr_y2', [-1,1])
b.add('fixed_distr_z2', [-1,1])
b.add('moving_distr_x2', [-1,1])
b.add('moving_distr_y2', [-1,1])
b.add('moving_distr_z2', [-1,1])
...
input_names = ['agent_x','agent_y','agent_z','cart_x'] + ['bread_'+str(i) for i in range(len(bread_positions))] + ['fixed_distr_x1', 'fixed_distr_y1', 'fixed_distr_z1', 'moving_distr_x1', 'moving_distr_y1', 'moving_distr_z1',
...
full_outcome = ['agent_x','agent_y','agent_z','cart_x',
                'fixed_distr_x1', 'fixed_distr_y1', 'fixed_distr_z1', 'moving_distr_x1', 'moving_distr_y1', 'moving_distr_z1',
                'fixed_distr_x2', 'fixed_distr_y2', 'fixed_distr_z2', 'moving_distr_x2', 'moving_distr_y2', 'moving_distr_z2', 'breads']
...
if args.model_type == "random_flat":
    ...
    outcome2 = ['cart_x']
    outcome3 = ['breads']
    outcome4 = ['fixed_distr_x', 'fixed_distr_y', 'fixed_distr_z']
    outcome5 = ['moving_distr_x', 'moving_distr_y', 'moving_distr_z']
    ...
    config = {'policy_nb_dims': total_policy_params,
              ...
              'modules':{'agent_final_pos':{'outcome_bounds':np.array(b.get_bounds(outcome1)),
                         ...
                         'fixed_dist_final_pos1':{'outcome_bounds':np.array(b.get_bounds([v+'1' for v in outcome4])),
                                            'outcome_range': np.array([full_outcome.index(var+'1') for var in outcome4])},
                         'moving_dist_final_pos1':{'outcome_bounds':np.array(b.get_bounds([v+'1' for v in outcome5])),
                                           'outcome_range': np.array([full_outcome.index(var+'1') for var in outcome5])},
                         'fixed_dist_final_pos2':{'outcome_bounds':np.array(b.get_bounds([v+'2' for v in outcome4])),
                                            'outcome_range': np.array([full_outcome.index(var+'2') for var in outcome4])},
                         'moving_dist_final_pos2':{'outcome_bounds':np.array(b.get_bounds([v+'2' for v in outcome5])),
                                           'outcome_range': np.array([full_outcome.index(var+'2') for var in outcome5])},
                                              }}
